...
Kosbling Edition 在 OpenClaw 官方配置基础上，新增 `edition` 配置块，用于定制化功能。所有 Kosbling 定制配置位于 `openclaw.json` 的 `edition` 顶级字段下。

当 `edition` 字段不存在或相关功能未启用时，行为与官方版本完全一致。
...
#### `edition.modelIsolation.enabled`

- 类型：`boolean`
- 默认：`false`
- 说明：是否启用模型隔离。设为 `false` 或不配置时，所有行为与官方版本一致。

#### `edition.modelIsolation.main`
...
#### `edition.modelIsolation.secondary`

- 类型：`object`
- 说明：secondary 组的模型配置。字段与 main 相同。

#### `edition.modelIsolation.agents`
